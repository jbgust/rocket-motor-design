...
public class ExportControler {
    ...
    public ResponseEntity exportRASP(@RequestBody ExportRASPRequest request) {
        JSRMResult jsrmResult = jsrmService.runComputation(request.getHollowComputationRequest(), request.isSafeKN()).getResult();
        ...
        response
                ...
                .append(format(jsrmResult.getGrainMassInKg(), "%.3f")).append(" ")
        ...
        jsrmResult.getMotorParameters().stream()
                .filter(motorParameters -> moduloLimitSize == 1 || i.getAndIncrement() % moduloLimitSize == 0)
                .forEach(motorParameters -> response
                .append("\n    ")
                .append(toFrontendPrecision(motorParameters.getTimeSinceBurnStartInSecond())).append(" ").append(toFrontendPrecision(motorParameters.getThrustInNewton())));
